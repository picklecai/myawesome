...
```
genspider后的参数，第一个是spider的名称，并不是项目名称，而是spider程序中`name=...`那个名称；第二个是待爬取的网址，会出现在spider程序中。但目前我还搞不清是第一个地址还是allowdomain，猜测是第一个地址。第二个网址写的时候，不要带`http://`以及最后的`/`，否则 `start_urls`的值就会比较怪异，因为它会自动添加上`http://`和最后的`/`，变成类似这样的表达： `start_urls = ['http://http://books.toscrape.com//']`
